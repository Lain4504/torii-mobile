...
class HandleMediaTracks {
  ...
  void localTrackUnpublished(
    ...
    if (kDebugMode) {
      print('HandleMediaTracks: Local track unpublished - ${track.source.name}');
    }
  }
  ...
  void trackSubscribed(
    ...
    if (kDebugMode) {
      print('HandleMediaTracks: Track subscribed - ${publication.source.name} from ${participant.identity}');
    }
  }
  ...
  void trackUnsubscribed(
    ...
    if (kDebugMode) {
      print('HandleMediaTracks: Track unsubscribed - ${publication.source.name} from ${participant.identity}');
    }
  }
  ...
  void trackSubscriptionFailed(
    ...
    if (kDebugMode) {
      print('HandleMediaTracks: Track subscription failed - $sid, error: $error');
    }
    
    // Show notification
    ref.read(roomSettingsProvider.notifier).addUserNotification(
      UserNotification(
        message: 'Không thể đăng ký track từ ${participant.name ?? participant.identity}',
        typeOption: 'error',
      ),
    );
  }
  ...
  void trackMuted(
    ...
    if (publication.source == TrackSource.microphone) {
      ...
      ref.read(participantProvider.notifier).updateParticipant(
        ...
        changes: {
          'metadata': {
            ...ref.read(participantProvider).participants[participant.identity]?.metadata.toJson() ?? {},
            'isMicMuted': true,
          },
        },
      );
    ...
    if (kDebugMode) {
      print('HandleMediaTracks: Track muted - ${publication.source.name} from ${participant.identity}');
    }
  }
  ...
  void trackUnmuted(
    ...
    if (kDebugMode) {
      print('HandleMediaTracks: Track unmuted - ${publication.source.name} from ${participant.identity}');
    }
  }
  ...
  void trackStreamStateChanged(
    ...
    if (kDebugMode) {
      print('HandleMediaTracks: Track stream state changed - ${publication.source.name} to ${streamState.name}');
    }
  ...
  void _addSpeaker(
    ...
    ref.read(activeSpeakersProvider.notifier).addOrUpdateSpeaker(
      userId: participant.identity,
  ...
  void _removeSpeaker(
    ...
    ref.read(activeSpeakersProvider.notifier).removeOneSpeaker(participant.identity);
