...
class CourseLiveDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: detailAsync.when(
        data: (detail) {
          if (detail == null) return const Center(child: Text('Không tìm thấy lớp'));
  ...
  Widget _buildContent(BuildContext context, ClassCatalogDetailModel detail) {
    ...
    final thumb = item.thumbnailUrl ?? 'https://picsum.photos/seed/course_live/800/600';
    final live = detail.liveClass;
    final canPay = live == null || !live.isLiveCapacityFull;
    final opening = item.term?['openingDate'];
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        if (item.jlptLevel != null) ...[
                          ...
                          Chip(label: Text(item.jlptLevel!, style: const TextStyle(fontSize: 12))),
                        ],
                        const Spacer(),
                    ...
                    Text(
                      item.name.isNotEmpty ? item.name : (item.profileTitle ?? 'Lớp học'),
                      style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
                    ),
                    const SizedBox(height: 8),
                    Text('Mã lớp: ${item.code}', style: TextStyle(color: AppColors.grey700, fontSize: 13)),
                    if (opening != null) ...[
                      const SizedBox(height: 8),
                      Text(
                        'Khai giảng: ${DateFormat('dd/MM/yyyy').format(DateTime.tryParse(opening.toString()) ?? DateTime.now())}',
                    ...
                    if (live?.liveCapacitySubtitle != null) ...[
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          color: live.isLiveCapacityFull ? AppColors.error : AppColors.grey700,
                    ...
                    const Text('Lịch & nội dung', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 10),
                    if (item.liveSchedules.isEmpty)
                      Text('Khung giờ học sẽ được cập nhật.', style: TextStyle(color: AppColors.grey700))
                    else
                    ...
                    if (detail.modules.isNotEmpty) ...[
                      const SizedBox(height: 16),
                      const Text('Chương trình (tham khảo)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      ...detail.modules.map((mod) => Padding(
                            padding: const EdgeInsets.only(bottom: 8),
                            child: Text('• ${mod['title'] ?? 'Chương'}', style: const TextStyle(fontSize: 13)),
                          )),
                    ],
